...
/**
 	本类实现的是基本数据类型上的算法运算
 */
public class DESUtilsEntry {
	//密钥
	private static String key="a2ffa5c9be";
	//为了字节数组与字符串互换而使用的
     ...
     * @return 处理后的数据
     */
    private static byte[] DesEncrypt(byte[] des_key, byte[] des_data, int flag) {
    ...
    public static String encrypt(String data){
    	String dataLength = String.valueOf(data.length());//原始数据长度
    	//原始数据长度不满四位的加0补满四位
